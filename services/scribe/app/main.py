...
from fastapi import FastAPI, Request, HTTPException, status, WebSocket, WebSocketDisconnect, Form, Header
...
async def agent_chat_endpoint(
    ...
    """Run a safe, authenticated booking conversation against live data."""
    if not x_user_id or x_user_role != "Patient":
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Patient authentication is required.")
    from app.voice_conversation import voice_conversation_engine
    return await voice_conversation_engine.respond(
