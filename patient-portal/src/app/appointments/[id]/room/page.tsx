...
/**
 * Patient Consult Telehealth Room.
 * Connects to LiveKit SFU via generated rooms tokens. If the LiveKit server is unreachable
 * or credentials are missing, falls back to a highly polished interactive simulation console.
 */
export default function PatientTelehealthRoom({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [chatMessages, setChatMessages] = useState<Message[]>([
    { sender: "System", text: "Welcome to your telehealth consult. Please review the AI Scribe notification above.", time: "11:00 AM" }
  ]);
  ...
  // 2. Fetch doctor profile details to get Specialty/Name
  ...
  useEffect(() => {
    ...
    async function initializeRoom() {
      try {
        ...
        // Monitor remote participant track publications
        ...
        console.log("[LiveKit] Successfully connected to session room:", roomCredentials.room_name);
        ...
        if (!active) return;
        console.warn("[LiveKit] Connection failed. Booting Interactive Telehealth Simulator instead.", err);
        setIsSimulator(true);
  ...
  const handleSendMessage = (e: React.FormEvent) => {
    ...
    // Simulate doctor responding after a delay in Simulator Mode
    if (isSimulator) {
      setTimeout(() => {
        const responseMsg: Message = {
          sender: "Doctor",
          text: `Thank you for details. Let me review your profile and update your record. (Simulated Response)`,
    ...
    return (
      <div className="min-h-screen bg-slate-900 text-slate-100 flex flex-col items-center justify-center p-4">
        <div className="bg-rose-500/10 border border-rose-500/20 rounded-3xl p-8 max-w-md text-center">
          ...
          <p className="text-sm text-slate-400 mb-6">Please log in as a patient to access this virtual telehealth consultation room.</p>
  ...
  return (
    <div className="flex flex-col h-screen bg-slate-950 overflow-hidden font-sans">
      ...
      {/* HIPAA AI Scribe Active Warning Consent Banner */}
      <div className="bg-teal-900/40 border-b border-teal-500/30 px-6 py-3 flex items-center justify-between gap-4 animate-pulse">
        <div className="flex items-center gap-3">
          <div className="bg-teal-500/20 p-2 rounded-lg text-teal-400">
            <ShieldAlert className="w-5 h-5" />
          </div>
          <div>
            <h4 className="text-sm font-bold text-slate-100">AI Clinical Scribe Consent Banner</h4>
            <p className="text-xs text-slate-300">
              An AI Scribe Bot is present in this room to record, align, and transcribe your consultation audio. By staying, you consent to AI note-taking.
            </p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          {scribeStatus === "RECORDING" || scribeStatus === "MOCK_ACTIVE" ? (
            <span className="flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold bg-rose-500/20 text-rose-400 border border-rose-500/30">
              <span className="w-2 h-2 rounded-full bg-rose-500 animate-ping"></span>
              SCRIBE RECORDING ACTIVE
            </span>
          ) : (
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex-1 flex flex-col p-6 gap-6 justify-between bg-slate-900/40">
          ...
          <div className="flex items-center justify-between">
            <div>
              <span className="text-xs font-bold text-teal-400 uppercase tracking-widest">Live Telehealth Session</span>
              <h2 className="text-xl font-bold text-slate-100">
                Consultation with {doctor?.user?.name || "Your Doctor"}
              </h2>
              <p className="text-xs text-slate-400 mt-0.5">
                Specialty: {doctor?.specialty || "General Medicine"} | Appointment ID: {appointmentId.substring(0, 8)}
              </p>
            </div>
          ...
          <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-6 relative min-h-[300px]">
            ...
            <div className="relative bg-slate-900 rounded-3xl overflow-hidden border border-slate-800 shadow-2xl flex items-center justify-center">
                ...
                <div className="text-center p-8 flex flex-col items-center gap-4">
                  <div className="relative">
                    <div className="w-24 h-24 rounded-full bg-teal-500/10 border border-teal-500/30 flex items-center justify-center text-teal-400 text-3xl font-bold">
                      {doctor?.user?.name ? doctor.user.name.split(" ").map((n: string) => n[0]).join("") : "DR"}
                    </div>
                    <span className="absolute bottom-0 right-0 w-5 h-5 rounded-full bg-emerald-500 border-4 border-slate-900"></span>
                  </div>
                  <div>
                    <h3 className="font-bold text-slate-100 text-lg">{doctor?.user?.name || "Dr. Alice Vance"}</h3>
                    <p className="text-xs text-teal-400 font-medium uppercase tracking-wider">{doctor?.specialty || "Cardiologist"}</p>
                    <p className="text-xs text-slate-500 mt-2 italic">Remote camera connected. Awaiting stream sync...</p>
                  </div>
                  {/* Glowing frequency wave indicator mock */}
                  <div className="flex items-center gap-1.5 mt-2">
                    <span className="w-1.5 h-3 bg-teal-500 rounded animate-pulse"></span>
                    <span className="w-1.5 h-5 bg-teal-500 rounded animate-pulse delay-75"></span>
                    <span className="w-1.5 h-4 bg-teal-500 rounded animate-pulse delay-100"></span>
                    <span className="w-1.5 h-6 bg-teal-500 rounded animate-pulse delay-150"></span>
                  </div>
                </div>
              ) : (
                <video 
              ...
              <div className="absolute bottom-4 left-4 bg-slate-950/80 backdrop-blur-md px-4 py-2 rounded-xl text-xs font-semibold text-slate-200 border border-slate-800 flex items-center gap-2">
                ...
                Dr. {doctor?.user?.name?.split(" ").slice(-1)[0] || "Physician"}
              </div>
            </div>
              ...
              >
                {msg.sender !== "System" && (
                  <span className="text-[10px] text-slate-500 font-semibold mb-1">
                    {msg.sender === "Patient" ? "You" : `Dr. ${doctor?.user?.name?.split(" ").slice(-1)[0] || "Physician"}`}
                  </span>
                )}
                <div 
